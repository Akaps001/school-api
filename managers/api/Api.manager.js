...
module.exports = class ApiHandler {
        ...
        Object.keys(this.managers).forEach(mk => {
            if (this.managers[mk][this.prop]) {
                ...
                this.managers[mk][this.prop].forEach(i => {
                    ...
                    console.log(`[ApiHandler Init] Registered: ${mk}.${fnName} [${method}]`);
                });
            }
        });
        ...
        Object.keys(this.managers).forEach(mk => {
            if (this.managers[mk].interceptor) {
                ...
                if (this.exposed[mk].cortexExposed) {
                    // console.log(`## ${mk} exposed via cortex`);
                }
            }
        });
    ...
    async mw(req, res, next) {
        ...
        if (!moduleMatrix[method].includes(fnName)) {
            return this.managers.responseDispatcher.dispatch(res, { ok: false, message: `unable to find function ${fnName} with method ${method} in module ${moduleName}` });
        }
